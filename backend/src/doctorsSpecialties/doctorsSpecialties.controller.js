...
const getDoctorsSpecialties = async (req, res) => {
  try {
    const doctorsSpecialties = await doctorsSpecialtiesUseCase.getDoctorsSpecialties();
...
const getDoctorsSpecialtyById = async (req, res) => {
  const { id } = req.params;
  try {
    const doctorsSpecialty = await doctorsSpecialtiesUseCase.getDoctorsSpecialtyById(id);
    if (!doctorsSpecialty) {
...
const updateDoctorsSpecialty = async (req, res) => {
  ...
  const { doctorId, specialtyId } = req.body;
  try {
    const doctorsSpecialty = await doctorsSpecialtiesUseCase.updateDoctorsSpecialtyById(id, { doctorId, specialtyId });
    if (!doctorsSpecialty) {
      return res.status(404).json({ message: "Doctors specialty not found" });
    }
    res.status(200).json(doctorsSpecialty);
...
const deleteDoctorsSpecialty = async (req, res) => {
  const { id } = req.params;
  try {
    const doctorsSpecialty = await doctorsSpecialtiesUseCase.deleteDoctorsSpecialtyById(id);
    if (!doctorsSpecialty) {
      return res.status(404).json({ message: "Doctors specialty not found" });
    }
